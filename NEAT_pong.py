...
def plot_training_progress(stats):
    ...
    best_fitness = [c.fitness for c in stats.most_fit_genomes.values()]
    avg_fitness = [s.fitness for s in stats.get_fitness_mean()]
...
if __name__ == '__main__':
    ...
    config = neat.Config(neat.DefaultGenome, neat.DefaultReproduction,
        ...
        # Set this variable to True for training or False for testing
    training_mode = True# Change this to False when you want to test

    if training_mode:
        run_neat(config)

         # Load the training statistics from the checkpoint
        stats = neat.StatisticsReporter()
        stats.load_stats('neat-checkpoint-7/stats')
